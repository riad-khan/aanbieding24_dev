        ...
        <div class="item" v-for="product in products">
            <nuxt-link :to="'/products/'+ generateSlug(product.product_title) + '-'+product.id"
                ...
                <div class="product-info mt-2 flex items-center justify-between">
                    <span class="w-fit bg-[#EBF1FF] text-[#0052FE] text-xs font-bold p-2 rounded " v-for ="category in splitComma(product.category_name)">{{ category }}</span>
...
const props = defineProps(["product_category"]);
const { data: products } = await useAsyncData('blog_products', () => $fetch(`${apiUrl}/blog-products/${props.product_category}`));




onMounted(() => {
    refreshNuxtData('blog_products');
})
</script>